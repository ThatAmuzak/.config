...
- [[#sane-emacs][SANE EMACS]]
  ...
  - [[#disable-confirmation-for-code-block-execution-in-org][Disable confirmation for code block execution in org]]
...
- [[#fonts][FONTS]]
  ...
  - [[#emoji--symbol-fallback][Emoji & Symbol Fallback]]
...
- [[#org-mode][ORG MODE]]
  ...
  - [[#minor-org-fixes][Minor Org Fixes]]
...
- [[#literate-programming][LITERATE PROGRAMMING]]
  - [[#babel-engines--persistent-sessions][Babel engines & persistent sessions]]
  - [[#code-block-helpers][Code block helpers]]
...
- [[#misc][MISC]]
  ...
  - [[#pidev-integration][Pi.dev integration]]
  - [[#adding-some-exec-paths][Adding some exec paths]]
...
** Disable confirmation for code block execution in org
...
Fix tofu boxes for emoji and Unicode symbols. Windows doesn't auto-fallback to emoji fonts like fontconfig does on Linux
...
Also enables inline images to be enabled by default
...
* PAPERS (Zotero library.bib -> org-roam)

Pipeline: Better BibTeX auto-exports ~/Notes/Brain/Papers/library.bib, PDFs
(named {citekey}.pdf) live in Papers/pdfs/, generated notes in Papers/notes/.
Citar is the fuzzy finder (orderless over author/year/title/journal/keywords);
notes are plain org files with :ID: and :ROAM_REFS: @citekey, so org-roam
indexes them as first-class nodes (SPC n f / SPC n i / org-roam-ref-find).
...
Remember to update elpaca menus melpa before running this, it's a pretty recent update
...
This is still probably worth having
...
Sources (because emacs 30.1 has not yet gotten treesitter version 15 compatible)
...
Some folding support with treesitter as well
...
Cuz yassnippets and company doesn't play well easily? Or I just couldn't get them to work properly
...
To add support for a new language, require its adapter below and add the relevant ~debug-template~ entries.
...
*** Rust / C / C++ (native debug)
Requires GDB 14+ or LLDB.
#+begin_src emacs-lisp
...
** Babel engines & persistent sessions
...
Auto save gptel buffers, incase I need to revisit them later on
...
Also just nicer scroll for normal scrolling
